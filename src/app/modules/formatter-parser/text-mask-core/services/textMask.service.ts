...
import {conformToMask} from "../conformToMask/conformToMask";
import adjustCaretPosition from "../adjustCaretPosition/adjustCaretPosition";
import {ITextMasResult} from "../text-mask-config";
...
export class TextMaskService {
  ...
  update(rawValue, config):ITextMasResult {
    let {
      inputElement,
      mask: providedMask,
      guide,
    ...
    this.state.previousConformedValue = inputElementValue; // store value for access for next time
    ...
    //this.safeSetSelection(inputElement, adjustedCaretPosition) // adjust caret position
  ...
  safeSetSelection(element, selectionPosition) {
    if (document.activeElement === element) {
      if (isAndroid) {
        defer(() => {
          element.setSelectionRange(selectionPosition, selectionPosition, 'none'), 0
        })
      } else {
